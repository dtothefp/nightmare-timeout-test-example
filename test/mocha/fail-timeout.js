...
describe('testing the create account dialog', function() {
  ...
  describe('filling out the create account form', function() {
    it('fails because password is not confirmed correctly', function(done) {
      new Nightmare({phantomPath: phantomPath})
        ...
        .screenshot(config.screenshot({ imgName: 'timeout-signup-form-filled' }))
        .wait(500)
        .screenshot(config.screenshot({ imgName: 'timeout-signup-complete' }))
        .evaluate(function() {
          return {
  ...
  describe('a test after a failure in mocha will timeout', function() {
    it('simple test runs and passes', function(done) {
      new Nightmare({phantomPath: phantomPath})
        ...
        .screenshot(config.screenshot({ imgName: 'timeout-signup-form-name-filled' }))
        .evaluate(function() {
